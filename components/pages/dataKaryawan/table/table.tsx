...
import { columns } from "@/components/pages/dataKaryawan/table/data"; // Import data
import { RenderCell } from "@/components/pages/dataKaryawan/table/render-cells"; // Import RenderCell function

/**
 * A table component that renders a list of perumahan items.
 *
 * @param { { items: DataKaryawanType[] } } props The props object.
 * @param { DataKaryawanType[] } props.items The list of perumahan items to render.
          ...
          <TableRow key={item.id_karyawan}>
            {(columnKey) => (
              <TableCell key={columnKey}>
                {RenderCell({ karyawan: item, columnKey })}
              </TableCell>
            )}
          </TableRow>
        )}
