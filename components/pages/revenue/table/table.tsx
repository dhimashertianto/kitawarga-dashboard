...
import { columns } from "@/components/pages/revenue/table/data"; // Import data
import { RenderCell } from "@/components/pages/revenue/table/render-cells"; // Import RenderCell function

/**
 * A table component that renders a list of perumahan items.
 *
 * @param { { items: ListPemasukanType[] } } props The props object.
 * @param { ListPemasukanType[] } props.items The list of perumahan items to render.
 * @returns { JSX.Element } The table component.
 */
export const TableWrapper: React.FC<{
  items: ListPemasukanType[];
}> = ({ items }) => {
          ...
          <TableRow key={item.id_transaksi}>
            {(columnKey) => (
              <TableCell key={columnKey}>
                {RenderCell({ pemasukan: item, columnKey })}
              </TableCell>
            )}
          </TableRow>
        )}
